...
class GetInfo(Model):
    ...
    def search_order(self, page, limit, sort, id=None, name=None):
        ...
        if id != None:
            self.cursor.execute(
                f"SELECT id,pmoney,scid,sgo,cid,go,`order`.wecharid,room_id,id_status,name FROM `order`,user WHERE `order`.wecharid = user.wecharid AND `order`.id={id} ORDER BY id DESC;")
        elif name != None:
            self.cursor.execute(
                f"SELECT id,pmoney,scid,sgo,cid,go,`order`.wecharid,room_id,id_status,name FROM `order`,user WHERE `order`.wecharid = user.wecharid AND user.name = '{name}' ORDER BY id DESC;")
        elif sort == '+id':
            self.cursor.execute(
